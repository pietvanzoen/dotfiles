...
4. For each logical group, check if it clearly belongs to an existing recent commit (e.g. same feature, same file, continuation of same work). If so, amend that commit with `git commit --amend --no-edit`. Only amend the most recent commit — for older commits, create a fixup commit with `git commit --fixup=<sha>`.
5. For new commits:
